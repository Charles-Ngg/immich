...
export class PersonService extends BaseService {
  ...
  async getThumbnail(auth: AuthDto, personGroupId: string): Promise<ImmichFileResponse> {
    ...
    const group = await this.personRepository.getForThumbnail({ ownerId: auth.user.id, personGroupId });
    const thumbnailPath = group?.thumbnailPath || group?.sharedThumbnailPath;
    if (!thumbnailPath) {
